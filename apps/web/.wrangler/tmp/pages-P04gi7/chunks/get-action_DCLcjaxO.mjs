...
import { A as AstroError, f as ActionNotFoundError } from './astro/assets-service_DOd1vnbN.mjs';

async function getAction(path) {
  const pathKeys = path.replace(/^.*\/_actions\//, "").split(".").map((key) => decodeURIComponent(key));
  let { server: actionLookup } = await import('./_astro_internal-actions_Bc-0wTFz.mjs');
