...
const mockTreks = [
  ...
  },
  {
    ...
    batches: [
      { id: "hp-b1", startDate: "2027-06-15", endDate: "2027-06-19", seatsTotal: 18, seatsBooked: 7, pricePerPersonINR: 11e3, status: "open" }
    ],
    packages: [
      { id: "hp-std", name: "Manali to Manali", priceINR: 11e3, description: "Complete package including Chandratal lake drive.", features: ["All inclusive trek", "Chandratal trip", "Atal Tunnel drive"], isPopular: true }
    ],
    isFeatured: true
  },
  {
  ...
  },
  {
    ...
    itinerary: [
      ...
      { day: 5, title: "Ruinsara Tal to Odari", altitudeM: 4e3, distanceKm: 4, description: "Steep rocky climb to the natural rock shelter of Odari." },
...
const STRAPI_URL = "https://civil-chaos.onrender.com";
const STRAPI_TOKEN = "";
...
async function getTreks() {
  const data = await fetchStrapi("/treks?populate=*&pagination[pageSize]=100");
  if (Array.isArray(data) && data.length > 0) {
    return data.map((item) => normalizeTrek(item));
  }
  return mockTreks;
}
function normalizeTrek(item) {
  ...
  return {
    ...
    coordinates: item.coordinates ? [item.coordinates.lng, item.coordinates.lat] : fallback.coordinates,
    region: item.region?.name || fallback.region,
    regionSlug: item.region?.slug || fallback.regionSlug,
    ...
    gallery: item.heroGallery?.map((g) => g.url) || fallback.gallery,
    altitudeProfile: fallback.altitudeProfile,
    itinerary: Array.isArray(item.itinerary) && item.itinerary.length > 0 ? item.itinerary.map((it) => ({
      day: it.day,
      title: it.title,
    ...
    batches: Array.isArray(item.batches) && item.batches.length > 0 ? item.batches.map((b) => ({
    ...
    faqs: Array.isArray(item.faq) && item.faq.length > 0 ? item.faq.map((f) => ({
      question: f.q,
      answer: f.a
    })) : fallback.faqs
  };
...
export { fetchStrapi as f, getTreks as g };
