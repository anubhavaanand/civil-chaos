...
const config = ({ env }: Core.Config.Shared.ConfigParams): Core.Config.Database => {
  const client = env('DATABASE_CLIENT', 'postgres'); // Default to postgres for Neon
  ...
  const connections: Record<Core.Config.Database.ClientKind, Core.Config.Database['connection']> = {
    ...
    postgres: {
      client: 'postgres',
      connection: {
        connectionString: env('DATABASE_URL'), // Neon pooled connection string
        host: env('DATABASE_HOST', 'ep-xxx.us-east-2.aws.neon.tech'),
